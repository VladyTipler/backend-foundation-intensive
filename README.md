...
## Готовая подготовка — не копируй сидеры из уроков

Открой **lab.cmd** на Windows или **lab.command** на macOS. Из терминала на любой системе: **npm run lab**.
В меню выбери номер дня. Оно проверит prerequisites, подготовит данные и не сотрёт твою практику. I устанавливает зависимости, U запускает инфраструктуру.

Подробности и наборы каждого дня: **[docs/lab-preparation.md](docs/lab-preparation.md)**.
Таблицы/миграции/маршруты/worker остаются твоими заданиями. Для аккаунтов Дня 3 сначала реализуй auth: helper регистрирует их через твой API, а не подсовывает готовые password hashes.
...
Не запускай `npm init` и не переписывай готовые файлы из урока: они уже в репозитории. Таблицы, колонки и миграции создавай самостоятельно по заданиям. Подготовку запускай после создания нужной структуры. Новые seed-команды добавляют недостающее и не очищают существующие данные; db:clean/reset — отдельные разрушительные действия.
...
Основной путь — меню **npm run lab**. Дни 1–2 используют один сохраняемый набор, Дни 3–7 добавляют личные данные двух учебных пользователей.
Индексы (X), N+1 (N), готовый файл для streams, тестовые аккаунты и реальные запросы/экспорты больше не требуют копирования вспомогательного кода.

Старые имена seed:base / seed:day1:indexes / seed:scenario:n-plus-one теперь вызывают новый **добавляющий** помощник. Они больше не очищают таблицы. Для X: 100000 новых вакансий, редкая компания Lab Rare Index Co (1%), частая Lab Common Index Co (80% новых строк).

Повтор не перезаписывает изменения ученика. Сохраняй локальный var/lab; пароли находятся только в accounts.private.json и не коммитятся. Подробнее — в [инструкции](docs/lab-preparation.md).
